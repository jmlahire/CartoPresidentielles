...
class SvgMapLayer extends SvgComponent {
    ...
    static type = '_Layer';
    static defaultOptions= { autofit:false, valuesKey:'values', blank:'#fff', clickable:true };

    constructor(id, options={}){
        super(id);
        if (svgMapRegister.has(id)) return svgMapRegister.get(id);
        else {
            this.options = { ...SvgMapLayer.defaultOptions, ...options };
            Object.assign(this.state, { rendered:false });
            this.container=d3.create('svg:g')
                .attr('id',this.id)
                .classed(SvgMapLayer.type,true)
