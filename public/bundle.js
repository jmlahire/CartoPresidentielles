(function () {
  ...
  function bin() {
    ...
    histogram.thresholds = function (_) {
      return arguments.length ? (threshold = typeof _ === "function" ? _ : Array.isArray(_) ? constant$5(slice.call(_)) : constant$5(_), histogram) : threshold;
    };
  ...
  function rank(values, valueof = ascending$1) {
    ...
    let k, r;
    Uint32Array.from(V, (_, i) => i).sort(valueof === ascending$1 ? (i, j) => ascendingDefined(V[i], V[j]) : compareDefined(compareIndex)).forEach((j, i) => {
  ...
  class Component extends Queue {
    ...
    fadeOut(options = {}) {
      ...
      this.enqueue(() => new Promise((resolve, reject) => {
        this.outerContainer.transition().duration(options.duration).delay(options.delay).style('opacity', 0).on('end', () => {
          if (options.type.display === 'visibility') this.invisiblify();else this.hide();
          resolve({
            msg: 'hidden',
            target: this
          });
    ...
    fadeIn(options = {}) {
      ...
      this.enqueue(() => new Promise((resolve, reject) => {
        this.outerContainer.style('display', 'block').transition().duration(options.duration).delay(options.delay).style('opacity', 1) // .on('start', this.outerContainer.style('display','block') )
        .on('end', () => resolve({
          msg: 'showed',
          target: this
        }));
  ...
  class ContentText extends Component {
    ...
    render() {
      ...
      const spans = Array.from(this.spans.values()).map(d => d.fcontent).join(' ');
  ...
  class selectItem extends abstractItem {
    ...
    deselect() {
      this.content.selectAll('option').filter(d => d).attr('selected', null);
      return this;
    }
  ...
  class NavButtons extends Component {
    ...
    render() {
      ...
      const figure = this.container.selectAll('figure').data(data).enter().append('figure').attr('class', d => `_${d[keys.value]}`).on('click', (e, d) => this.select(d));
      const img = figure.append('img').classed('photo', true).attr('src', d => `${options.path}${d[keys.img]}.${options.ext}`).attr('alt', d => d[keys.label]);
    ...
    select(datum) {
      ...
      this.container.selectAll('figure').classed('selected', d => d === datum);
      this.dispatch.call('select', datum, {
        value: datum[keyValue]
      });
      return this;
    }
  ...
  class Panel extends Component {
    ...
    constructor(id, options) {
      ...
      this._handle.on('click', e => {
        if (this.state.position === 'folded') this.unfold();else this.fold();
      });
    }
    ...
    resize() {
      ...
      this._handle.append('span').style('line-height', `${this.size.height}px`).style('font-size', `${this.size.handle * .5}px`).text('◀'); //Dimensionnement et positionnement contenu


      this.size.content = this.size.width - this.size.handle;

      this._innerContainer.style('left', `${this.size.handle}px`).style('width', `${this.size.width - this.size.handle}px`).style('height', `${this.size.height}px`); //Position initiale


      if (this.options.initialPosition === 'unfolded') this.outerContainer.classed('unfolded', true).style('right', '0px');else {
    ...
    fold(options = {}) {
      ...
      this.timer.stop();
      this.enqueue(() => new Promise((resolve, reject) => {
        this.outerContainer.transition().duration(options.duration).delay(options.delay).style('right', `-${this.size.content}px`).on('end', () => {
          this.dispatch.call('position', this, {
            action: 'folded'
          });
    ...
    unfold(options = {}) {
      ...
      this.enqueue(() => new Promise((resolve, reject) => {
        this.outerContainer.transition().duration(options.duration).delay(options.delay).style('right', '0px').on('end', () => {
          this.dispatch.call('position', this, {
            action: 'unfolded'
          });
  ...
  const d3$3 = Object.assign({}, d3Selection);
  ...
  class Svg extends SvgComponent {
    ...
    constructor(id, size = {}) {
      ...
      this._outerContainer = d3$3.create('svg:svg').attr('id', this.id).attr('class', this.constructor.type).attr(`preserveAspectRatio`, 'xMaxYMin meet').attr('viewBox', `0 0 ${this.size.width} ${this.size.height}`).attr('width', `100%`);
  ...
  class MapComposition extends Svg {
    ...
    constructor(id, size = {}, options = {}) {
      ...
      this.dispatch = d3$2.dispatch('zoom');
  ...
  class MapLayer extends SvgComponent {
    ...
    get path() {
      return this.parentComponent.path || d3$1.geoMercator();
    }

    get projection() {
      return this.parentComponent.projection || d3$1.geoPath();
    }
    /**
    ...
    render() {
      this.enqueue(() => new Promise((resolve, reject) => {
        ...
        this.container.selectAll("path").data(this.geodata).enter().append('path').attr('class', d => this._getId(d)).classed('area', true).classed('clickable', this.options.clickable).attr('d', this.path).on('click', (e, d) => this.options.clickable ? this.dispatch.call('click', this, {
          event: e,
          values: d.properties,
          id: d.properties[this.options.primary]
        }) : null); //  if (this.options.zoomable) this.parentComponent.zoomable(true);
    ...
    clickable(bool = true) {
      ...
      if (bool) paths.classed('clickable', true).on('click', (e, d) => this.dispatch.call('click', this, {
        event: e,
        values: d.properties,
        id: d.properties[this.options.primary]
    ...
    zoomOn(geoId) {
      ...
      if (geoId !== undefined) this.parentComponent.zoomTo(this.container.selectAll(`path.area._${geoId}`));else this.parentComponent.zoomTo(d3$1.select(`path.${this.id}`));
      return this;
    }
      ...
      this.enqueue(() => new Promise((resolve, reject) => {
        this.container.selectAll("path.area").each((d, i, n) => {
          ...
          if (data && this.options.clickable) {
            elt.style('fill', color).classed('clickable', true).on('click', (e, d) => {
              this.select(d).dispatch.call('click', this, {
                event: e,
                values: d.properties,
                id: d.properties[this.options.primary]
              });
